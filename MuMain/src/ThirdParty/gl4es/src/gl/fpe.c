...
void realize_glenv(int ispoint, int first, int count, GLenum type, const void* indices, scratch_t* scratch) {
    ...
    if(gl4es_glIsProgram(glstate->glsl->program)) {
        ...
        const int es_passthrough =
            (glprogram->last_vert && glprogram->last_vert->es_passthrough) ||
            (glprogram->last_frag && glprogram->last_frag->es_passthrough);
        if(!es_passthrough && glprogram->default_vertex) {
        ...
        } else if(!es_passthrough && !fpe_IsEmpty(&state))
        {
