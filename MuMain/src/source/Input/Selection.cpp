...
namespace
{
    int s_lastGamepadTarget = -1;

    bool TryProjectGamepadTarget(int index, int& outScreenX, int& outScreenY)
    {
        ...
        constexpr BYTE TargetKinds = KIND_MONSTER | KIND_EDIT | KIND_PLAYER;
        if (character == Hero
            || !object->Live
            || !object->Visible
            || object->Alpha <= 0.0f
            || character->Dead != 0
            || !character->m_bIsSelected
            || (TargetKinds & object->Kind) != object->Kind
            || g_isCharacterBuff(object, eBuff_CrywolfNPCHide))
        {
            return false;
        }

        vec3_t targetPoint;
        VectorCopy(object->Position, targetPoint);
        targetPoint[2] += 120.0f;
        CameraProjection::WorldToScreen(g_Camera, targetPoint, &outScreenX, &outScreenY);
        return outScreenX >= 0
            && outScreenX < GetScreenWidth()
            && outScreenY >= 0
            && outScreenY < 429;
    }
}
...
{
    if (SceneFlag != MAIN_SCENE || CharactersClient == nullptr || Hero == nullptr)
        return false;

    const int first = (s_lastGamepadTarget + 1 + MAX_CHARACTERS_CLIENT) % MAX_CHARACTERS_CLIENT;

    for (int offset = 0; offset < MAX_CHARACTERS_CLIENT; ++offset)
    {
        const int index = (first + offset) % MAX_CHARACTERS_CLIENT;
        if (!TryProjectGamepadTarget(index, outScreenX, outScreenY))
            continue;

        s_lastGamepadTarget = index;
        SelectedCharacter = index;
        return true;
    }

    s_lastGamepadTarget = -1;
    return false;
}

bool ProjectGamepadTarget(int& outScreenX, int& outScreenY)
{
    if (!TryProjectGamepadTarget(s_lastGamepadTarget, outScreenX, outScreenY))
    {
        s_lastGamepadTarget = -1;
        return false;
    }
    SelectedCharacter = s_lastGamepadTarget;
    return true;
}
