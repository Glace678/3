...
public class ItemConsumptionTest
{
    ...
    /// <param name="solo">Whether this player uses the solo profile.</param>
    ...
    public async ValueTask JewelOfLifeAsync(int numberOfOptions, bool consumptionExpectation, bool solo)
    {
        var consumeHandler = new LifeJewelConsumeHandlerPlugIn();
        consumeHandler.Configuration.SuccessChance = solo ? 0 : 1;
        var player = await this.GetPlayerAsync().ConfigureAwait(false);
        if (solo)
        {
            player.GameContext.Configuration.GlobalBaseAttributeValues.Add(
                new ConstValueAttribute(1, new AttributeDefinition(SoloBalance.ProfileAttributeId, "Solo", string.Empty)));
        }
        var upgradeableItem = this.GetItemWithPossibleOption();
        var upgradableItemSlot = (byte)(ItemSlot + 1);
        await player.Inventory!.AddItemAsync(upgradableItemSlot, upgradeableItem).ConfigureAwait(false);
        ...
        Assert.That(consumeHandler.Configuration.SuccessChance, Is.EqualTo(solo ? 0 : 1), "Per-player overrides must not mutate shared plug-in settings.");
        if (jolConsumed)
        {
