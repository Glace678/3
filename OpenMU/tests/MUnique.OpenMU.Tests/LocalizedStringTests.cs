...
public class LocalizedStringTests
{
    ...
    /// <summary>
    /// Tests that region-specific translations don't collide through their two-letter language code.
    /// </summary>
    [Test]
    public void GetTranslation_PrefersExactRegionalCulture()
    {
        var localized = new LocalizedString("Base||zh=旧版中文||zh-CN=简体中文||zh-TW=繁體中文");

        Assert.That(localized.GetTranslation(new CultureInfo("zh-CN")), Is.EqualTo("简体中文"));
        Assert.That(localized.GetTranslation(new CultureInfo("zh-TW")), Is.EqualTo("繁體中文"));
    }

    /// <summary>
    /// Tests that old two-letter translations remain readable when no exact regional translation exists.
    /// </summary>
    [Test]
    public void GetTranslation_FallsBackToLegacyTwoLetterCulture()
    {
        var localized = new LocalizedString("Base||zh=旧版中文");

        var result = localized.GetTranslation(new CultureInfo("zh-CN"));

        Assert.That(result, Is.EqualTo("旧版中文"));
    }
    ...
    /// <summary>
    /// Tests that region-specific translations are stored under their full BCP-47 culture names.
    /// </summary>
    [Test]
    public void WithTranslation_StoresRegionalCulturesSeparately()
    {
        var localized = new LocalizedString("Base");

        var withSimplified = localized.WithTranslation(new CultureInfo("zh-CN"), "简体中文");
        var withTraditional = withSimplified.WithTranslation(new CultureInfo("zh-TW"), "繁體中文");

        Assert.That(withTraditional.Value, Is.EqualTo("Base||zh-CN=简体中文||zh-TW=繁體中文"));
    }
