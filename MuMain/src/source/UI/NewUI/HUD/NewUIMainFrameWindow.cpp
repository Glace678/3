...
bool SEASON3B::CNewUISkillList::SelectAdjacentHotKeySkill(int direction)
{
    if (direction == 0 || Hero == nullptr || CharacterAttribute == nullptr)
        return false;

    int currentSlot = -1;
    for (int i = 0; i < SKILLHOTKEY_COUNT; ++i)
    {
        if (m_iHotKeySkillType[i] == Hero->CurrentSkill)
        {
            currentSlot = i;
            break;
        }
    ...
    for (int offset = 1; offset <= SKILLHOTKEY_COUNT; ++offset)
    {
        int slot = currentSlot + step * offset;
        slot %= SKILLHOTKEY_COUNT;
        if (slot < 0)
            slot += SKILLHOTKEY_COUNT;

        const int skillIndex = m_iHotKeySkillType[slot];
        if (skillIndex < 0 || CharacterAttribute->Skill[skillIndex] == 0)
            continue;

        const int before = Hero->CurrentSkill;
        UseHotKey(slot);
        return Hero->CurrentSkill != before;
    }
