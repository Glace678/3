...
namespace SEASON3B
{
    ...
    {
        inline constexpr std::uint32_t FieldFocusId = 0;
        inline constexpr std::uint32_t FirstItemFocusId = 1;

        constexpr int ClampIndex(int index, int itemCount)
        {
            if (itemCount <= 0)
                return 0;
            if (index < 0)
        ...
        constexpr int StepIndex(int index, int itemCount, int delta)
        {
            return ClampIndex(index + delta, itemCount);
        }

        constexpr int PageIndex(int index, int itemCount, int visibleItems, int direction)
        {
            const int pageSize = visibleItems > 0 ? visibleItems : 1;
            return StepIndex(index, itemCount, direction * pageSize);
        }

        constexpr std::uint32_t ItemFocusId(int index)
        {
            return FirstItemFocusId + static_cast<std::uint32_t>(index);
        }

        constexpr int ItemIndexFromFocusId(std::uint32_t focusId, int itemCount)
        {
            if (focusId < FirstItemFocusId || itemCount <= 0)
                return -1;
            const std::uint32_t index = focusId - FirstItemFocusId;
            return index < static_cast<std::uint32_t>(itemCount)
    ...
    public:
        ...
        bool IsFocusRestorePending() const { return m_bRestoreFieldFocus; }
        void Close();
        void CloseAndRestoreFocus();

        // Registers the closed field, or only the visible list rows while open.
        // The owner must omit its other focus targets while a combo is expanded.
    ...
    private:
        ...
        bool m_bOpen = false;
        bool m_bLockHighlightedFocus = false;
        bool m_bRestoreFieldFocus = false;
        int m_iHeldNavigationDelta = 0;
        std::uint64_t m_uNextNavigationRepeatMs = 0;
        ...
        void FocusField();
        void FocusHighlightedItem();
        int GetFocusedItemIndex() const;
        int GetNavigationDelta() const;
        bool ShouldApplyNavigation(int delta);
        bool UpdateControllerNavigation();
        bool CommitSelection(int idx);
    };
}
