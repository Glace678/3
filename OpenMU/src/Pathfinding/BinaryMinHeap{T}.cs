...
public class BinaryMinHeap<T> : IPriorityQueue<T>
{
    ...
    public void Push(T item)
    {
        ...
        do
        {
            ...
            if (this.OnCompareWithElement(index, parentIndex) < 0)
            {
                this.SwitchElements(index, parentIndex);
                index = parentIndex;
            }
            else
            {
    ...
    public T Pop()
    {
        ...
        do
        {
            ...
            if (this._innerList.Count > left && this.OnCompareWithElement(index, left) > 0)
            {
                index = left;
            }

            if (this._innerList.Count > right && this.OnCompareWithElement(index, right) > 0)
            {
                index = right;
            }
    ...
    private void SwitchElements(int index, int parentIndex)
    {
        T h = this._innerList[index];
        this._innerList[index] = this._innerList[parentIndex];
        this._innerList[parentIndex] = h;
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private int OnCompareWithElement(int index, int otherIndex)
    {
        return this._elementComparer.Compare(this._innerList[index], this._innerList[otherIndex]);
    }
}