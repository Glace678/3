...
static Core::Time::ReferenceTickAccumulator g_actionObjectTicks;
...
bool Calc_RenderObject(OBJECT* o, bool Translate, int Select, int ExtraMon)
{
    ...
    {
        o->Alpha -= Core::Time::ScaleLinearStep(0.15f, FPS_ANIMATION_FACTOR);
    }
                            ...
                            if (gMapManager.WorldActive == WD_73NEW_LOGIN_SCENE)
                            {
                                    ...
                                    TestFrustrum2D(o->Position[0] * 0.01f, o->Position[1] * 0.01f, -500.f) && fDistance < fDis * 2.0f)
                                {
                                    ...
                                    if (o->AlphaTarget < 1.0f)
                                        o->AlphaTarget += 0.03f * FPS_ANIMATION_FACTOR;
                                    else
                                        o->AlphaTarget = 1.0f;
...
{
    int     objectCount = 0;
    const bool updateActionObjects = g_iActionTime >= 0
        && g_actionObjectTicks.Advance(FPS_ANIMATION_FACTOR) > 0;
    ...
    if (updateActionObjects && g_iActionTime >= 0)
        g_iActionTime--;
}
