...
{
    /// <summary>Checks the migration snapshot and wallet optimistic concurrency mapping without opening a database.</summary>
    [Test]
    public void SoloShopLedgerHasMigrationAndConcurrencyProtection()
    {
        using var context = new EntityDataContext();
        var property = context.Model.FindEntityType(typeof(Account))!.FindProperty(nameof(Account.SoloCashShopData));
        Assert.That(property, Is.Not.Null);
        Assert.That(property!.IsConcurrencyToken, Is.True);
        Assert.That(property.IsNullable, Is.False);
        Assert.That(context.Database.GetMigrations(), Does.Contain("20260905000100_AddSoloCashShopData"));
