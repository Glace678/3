...
bool CNewUISystem::Create()
{
    ...
    if (m_pNewOptionWindow->Create(m_pNewUIMng, (640 / 2) - (570 / 2), 5) == false)
    {
        return false;
    }
...
bool CNewUISystem::HasVisibleGamepadInterface()
{
    if (!m_pNewUIMng)
    {
        return false;
    ...
    for (int key = INTERFACE_BEGIN + 1; key < INTERFACE_END; ++key)
    {
        ...
        // These are persistent HUD/read-only layers. Treating them as modal UI
        // would disable world movement whenever the player enters the game.
        switch (key)
        {
        ...
        if (key >= INTERFACE_3DRENDERING_CAMERA_BEGIN
            && key <= INTERFACE_3DRENDERING_CAMERA_END)
        {
            continue;
        }
