...
public abstract class ItemModifyConsumeHandlerPlugIn : BaseConsumeHandlerPlugIn
{
    ...
    public override async ValueTask<bool> ConsumeItemAsync(Player player, Item item, Item? targetItem, FruitUsage fruitUsage)
    {
        ...
        if (!this.ModifyItem(player, targetItem))
        {
            return false;
        }
    ...
    /// <summary>Modifies an item using the current player's game configuration.</summary>
    /// <param name="player">The consuming player.</param>
    /// <param name="item">The target item.</param>
    /// <returns>Whether the item accepted the modification.</returns>
    protected virtual bool ModifyItem(Player player, Item item) =>
