...
{
    const auto* package = FindDisplayPackage(iIndex);
    return package ? static_cast<WORD>(_wtoi(package->InGamePackageID)) : static_cast<WORD>(-1);
}

const CShopPackage* CInGameShopSystem::FindDisplayPackage(int index) const
{
    if (index < 0 || static_cast<size_t>(index) >= m_listDisplayPackage.size())
        return nullptr;
    return &*std::next(m_listDisplayPackage.begin(), index);
}

int CInGameShopSystem::GetPackageItemLevel(int iIndex) const
{
    constexpr int LevelsPerSoloItem = 16;
    const auto* package = FindDisplayPackage(iIndex);
    if (!GameConfig::GetInstance().IsSoloBalanceEnabled() || !package || package->PackageProductSeq <= 0)
        return 0;
    return (package->PackageProductSeq - 1) % LevelsPerSoloItem;
}
...
{
    return GameConfig::GetInstance().IsSoloBalanceEnabled() || !m_bIsShopOpenLock;
}
