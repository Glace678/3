...
| `OPENMU_CONNECTION_SETTINGS_FILE` | Absolute or working-directory-relative path to a complete `ConnectionSettings.xml` replacement. This takes precedence over the file beside the persistence assembly. Protect the file because it contains database credentials. |
| `OPENMU_BIND_ADDRESS` | IPv4 address used by the connect, game, and chat TCP listeners. If unset, the legacy default is all IPv4 interfaces. Set this to `127.0.0.1` for a local-only stack. Invalid and IPv6 values fail startup instead of widening the listener. This is independent of `-resolveIP`, which only controls the address reported to clients. |
| `OPENMU_CONTROL_PIPE` | Optional Windows named pipe name for the local `status` and graceful `shutdown` commands. The pipe is created with current-user-only access. Intended for the bundled local launcher. |
