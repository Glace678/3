...
{
    static Core::Time::FramePacer framePacer;
    (void)usePreciseSleep;

    // Refresh the remaining time immediately before waiting. The main loop can
    // spend measurable time pumping input and network queues after its first
    // frame-clock sample, especially at 120-180 FPS.
    g_frameTiming.UpdateCurrentTime(g_pTimer->GetTimeElapsed());
    framePacer.WaitFor(g_frameTiming.GetTimeUntilNextFrame());
}
