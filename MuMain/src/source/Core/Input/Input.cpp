...
void CInput::Update()
{
    ...
    if (m_bHasCursorOverride)
    {
        m_ptCursor = m_ptCursorOverride;
        m_bHasCursorOverride = false;
    }
    else
    {
        ::GetCursorPos(&m_ptCursor);
    ...
    if (m_bLeftHand)
    {
        ...
        else if ((SEASON3B::IsRelease(VK_LBUTTON) || SEASON3B::IsNone(VK_LBUTTON))
            && m_bRBtnHeldDn)
        {
        ...
        else if ((SEASON3B::IsRelease(VK_RBUTTON) || SEASON3B::IsNone(VK_RBUTTON))
            && m_bLBtnHeldDn)
        {
    ...
    else if ((SEASON3B::IsRelease(VK_MBUTTON) || SEASON3B::IsNone(VK_MBUTTON))
        && m_bMBtnHeldDn)
    {
    ...
    bool forceAcceptanceFocus = false;
#if defined(MU_ENABLE_VIRTUAL_GAMEPAD_TESTS)
    static const bool configuredForceAcceptanceFocus = [] {
        const char* value = std::getenv("MU_VIRTUAL_GAMEPAD_FORCE_FOCUS");
        return value != nullptr && std::strcmp(value, "1") == 0;
    }();
    forceAcceptanceFocus = configuredForceAcceptanceFocus;
#endif
    if (GetActiveWindow() == NULL && !forceAcceptanceFocus)
    {
        m_lDX = m_lDY = 0L;
        m_bLBtnUp = m_bRBtnUp = m_bMBtnUp = false;
        m_bLBtnDn = m_bRBtnDn = m_bMBtnDn = false;
