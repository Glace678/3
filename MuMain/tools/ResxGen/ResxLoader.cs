...
internal static class ResxLoader
{
        ...
        Dictionary<string, Dictionary<string, ResxRawEntry>> perLocale)
    {
        ...
        foreach (var (key, defaultRaw) in defaultEntries)
        {
            ...
            if (identifier.Length == 0 && defaultRaw.LegacyIds.Count > 0)
            {
                identifier = $"Legacy{defaultRaw.LegacyIds[0]}";
            }

            if (identifier.Length == 0)
            {
                // Resource-only entries without an ASCII key or legacy ID
                // cannot be addressed safely from generated C++.
                Console.Error.WriteLine(
                    $"Skipping {groupName} entry with no ASCII identifier or legacy ID: '{key.Substring(0, Math.Min(40, key.Length))}'");
                continue;
            }
