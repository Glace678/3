...
#include "GameShop/SoloShopCatalog.h"
...
public:
    ...
    bool ScriptDownload();
    bool BannerDownload();
    void BeginSoloCatalog(int count);
    void AppendSoloOffer(const GameShop::SoloCatalog::Offer& offer);
    void FinishSoloCatalog();
    bool IsSoloCatalogReady() const { return m_soloCatalogReady; }
...
public:
    ...
    int GetPackageItemLevel(int iIndex) const;
...
protected:
    const CShopPackage* FindDisplayPackage(int index) const;
    void InitZoneInfo();
...
protected:
    ...
    int m_soloExpectedOffers = -1;
    int m_soloReceivedOffers = 0;
    bool m_soloCatalogReady = false;
